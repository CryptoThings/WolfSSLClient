...
#ifdef WOLFSSL_SMALL_STACK
  if (x->used > (n->used * 2 + 1)) {
    W = (mp_word*)XMALLOC(sizeof(mp_word) * x->used /*MP_WARRAY=512*/, 0, DYNAMIC_TYPE_BIGINT);
  } else {
    W = (mp_word*)XMALLOC(sizeof(mp_word) * (n->used * 2 + 1) /*MP_WARRAY=512*/, 0, DYNAMIC_TYPE_BIGINT);
  }
  if (W == NULL) {
/*
    void *fm;
    ...
    char serr[32];
    do {
      snprintf(serr, 32, "alloc %d", _x);
      WOLFSSL_MSG(serr);
      fm = malloc(_x);
      _x += 128;
      free(fm);
    } while (fm != NULL);
    WOLFSSL_MSG("fast_mp_montgomery_reduce malloc");
    snprintf(serr, 32, "size %d * %d %d", sizeof(mp_word), x->used, n->used);
    WOLFSSL_MSG(serr);
*/
    return MP_MEM;
  }
