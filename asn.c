...
int ParseCertRelative(DecodedCert* cert, int type, int verify, void* cm)
{
 ...
 if (verify && type != CA_TYPE && type != TRUSTED_PEER_TYPE) {
        Signer* ca = NULL;
        WOLFSSL_MSG("cert:");
//        WOLFSSL_MSG(cert->subjectCN);
        WOLFSSL_MSG(cert->issuer);
        WOLFSSL_MSG(cert->subject);
        WOLFSSL_MSG("GetCA:");
...
static int MakeSignature(const byte* buffer, int sz, byte* sig, int sigSz,
                         ...
                         int sigAlgoType, CallbackEccSignCert sign_cb, void *ctx)
{
    ...
    if (!rsaKey && eccKey) {
        ...
        if (sign_cb == NULL) {
            ...
            ret = (*sign_cb)(digest, digestSz, sig, &outSz, rng, ctx);
        }
...
int wc_SignCert(int requestSz, int sType, byte* buffer, word32 buffSz,
             RsaKey* rsaKey, ecc_key* eccKey, WC_RNG* rng)
{
  return wc_SignCert_cb(requestSz, sType, buffer, buffSz,
             rsaKey, eccKey, rng, NULL, NULL);
}

int wc_SignCert_cb(int requestSz, int sType, byte* buffer, word32 buffSz,
             RsaKey* rsaKey, ecc_key* eccKey, WC_RNG* rng, CallbackEccSignCert sign_cb, void *ctx)
{
    int sigSz;
#ifdef WOLFSSL_SMALL_STACK
    byte* sig;
...
int wc_MakeSelfCertEcc(Cert* cert, byte* buffer, word32 buffSz,
